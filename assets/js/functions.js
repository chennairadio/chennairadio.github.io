...
function getTotalTimeFromCurrentDate(){
	...
	var finalDiffDays = Math.floor(Math.abs((dateToLaunch.getTime() - currentDate.getTime())/(oneDay)));
	var hourOffsetInDays = (Math.abs(dateToLaunch.getTime() - currentDate.getTime())/oneDay) - finalDiffDays;
	var finalDiffHours = Math.floor(hourOffsetInDays * 24);
	var minutesDiff = (hourOffsetInDays * 24) - finalDiffHours;
	var finalDiffMinutes = Math.round(minutesDiff * 60);
	return {
		days : finalDiffDays,
		hours : finalDiffHours,
		minutes : finalDiffMinutes
	};
	}
	// var hoursDiff = 23 - currentDate.getHours();
	// var daysDiff =
var timeOffset = getTotalTimeFromCurrentDate();
	/* Time Countdown
	-------------------------------------------------------------------*/
	$('#time_countdown').countDown({
        ...
        // omitWeeks: true
         ...
         targetOffset: {
            ...
            'month':    0,
            'year':     0,
            'hour':     timeOffset.hours,
            'min':      timeOffset.minutes,
            'sec':      0
		},
		omitWeeks: true
